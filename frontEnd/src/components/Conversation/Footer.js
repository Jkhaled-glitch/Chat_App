...
const Footer = ({convId}) => {
    ...
    const handleSendMessage = async() => {
       ...
       try{
        ...
        const response = await axios.post("http://localhost:8080/api/messages/publish",
        {
            content: message,
            conv_id: convId
        },{
    ...
    <Stack direction='row' alignItems={'center'} spacing={3}>
        ...
        <Stack sx={{width:'100%'}}> 
            ...
            <Box sx={{ display: openPicker ? 'inline' : 'none' , zIndex:10, position:'fixed',bottom:81, right:100}}>
                <Picker theme={theme.palette.mode} data={data} onEmojiSelect={(emoji)=>setMessage((message)=>message + emoji.native)}/>
            </Box> 
